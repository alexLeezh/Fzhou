...
function bef_project($deal_id,$user_id)
{
	if($deal_id=='')
	{
		$results=$GLOBALS['db']->getAll("select * from ".DB_PREFIX."deal where is_effect = 0 and is_edit = 1 and is_delete = 0 and user_id = ".$user_id);
		if($results)
		{
			foreach($results as $k=>$v)
			{
				$deal_item_count = $GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."deal_item where deal_id = ".$results[$k]["id"]);
				
				if($v["description"]=='' || $v["xmdo"]=='' || $v["xmld"]=='')
				{
					return "project#add_explain&id=".$v["id"];
				}
				else if($v["jihua"]=='' || $v["yusuan"]=='' || $v["youshi"]=='')
				{
					return "project#zhixing&id=".$v["id"];
				}
				else if($v["name"]=='')
				{
					return "project#add_info&id=".$v["id"];
				}
				else if($deal_item_count==0)
				{
					return "project#add_item&id=".$v["id"];
				}
			}
		}
		else
		{
			return "project#add_easy";
		}
	}
	else
	{
		$deal_item = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."deal where id = ".$deal_id." and is_effect = 0 and is_delete = 0 and user_id = ".$user_id);
		if($deal_item)
		{
			$deal_item_count = $GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."deal_item where deal_id = ".$deal_id);
			
			if($deal_item["description"]=='' || $deal_item["xmdo"]=='' || $deal_item["xmld"]=='')
			{
				return "project#add_explain&id=".$deal_id;
			}
			else if($deal_item["jihua"]=='' || $deal_item["yusuan"]=='' || $deal_item["youshi"]=='')
			{
				return "project#zhixing&id=".$deal_id;
			}
			else if($deal_item["name"]=='')
			{
				return "project#add_info&id=".$deal_id;
			}
			else if($deal_item_count==0)
			{
				return "project#add_item&id=".$deal_id;
			}
		}
	}
}
