...
class indexModule extends BaseModule
{
	...
	{	
		...
		$cate_hot_result = load_dynamic_cache("INDEX_HOT_CATE_LIST");
		if($cate_hot_result===false)
		{
			$cate_hot_list = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."deal_cate where hot = 1 order by sort asc");
			$cate_hot_result= array();
			foreach($cate_hot_list as $k=>$v)
			{
				$cate_hot_result[$v['id']] = $v;
			}
			set_dynamic_cache("INDEX_HOT_CATE_LIST",$cate_hot_result);
		}
		$GLOBALS['tmpl']->assign("cate_hot_list",$cate_hot_result);
		...
		if($r!="")
		{
			...
			if($r=="new")
			{
				...
				$GLOBALS['tmpl']->assign("page_title","最新项目");
			}
			if($r=="classic")
			{
				$condition.=" and is_classic = 1 ";
				$GLOBALS['tmpl']->assign("page_title","本周热门");
			}
			if($r=="success")
			{
				$condition.=" and is_success = 1 ";
				$GLOBALS['tmpl']->assign("page_title","最近成功");
			}
			if($r=="soon")
			{
				$condition.=" and end_time > ".NOW_TIME." and support_amount - limit_price >= 200 ";  //时间大于当前时间并且所筹金额不能低于需筹金额200为即将成功
				$GLOBALS['tmpl']->assign("page_title","即将成功");
			}
			if($r=="end")
			{
		...
		foreach($deal_list as $k=>$v)
		{
			...
			$deal_list[$k]['tags_arr'] = preg_split("/[ ,]/",$v['tags']);
		}
