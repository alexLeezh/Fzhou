...
class settingsModule extends BaseModule
{
	...
	public function account()
	{
		...
		$GLOBALS['tmpl']->display("settings_account.html");
	}
	...
	{		
		if(!$GLOBALS['user_info'])
		app_redirect(url("user#login"));		
		$GLOBALS['tmpl']->display("settings_incharge.html");
	}
	...
	{		
		...
		showSuccess("",$ajax,url("settings#pay",array("money"=>round($money*100))));
	}
	...
	{		
		...
		$user_oldpwd = strim($_REQUEST['user_oldpwd']);
		$user_data = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."user where id = ".intval($GLOBALS['user_info']['id']));
		if($user_data['user_pwd'] != md5($user_oldpwd.$user_data['code'])&&$user_data['user_pwd']!=$user_oldpwd)
		{
			showErr("旧密码错误",$ajax,"");
		}
	...
	{		
		...
		if($ex_real_name==""||$ex_account_info==""||$ex_contact==""||$ex_qq==""||$ex_account_bank=="")
		{
			showErr("请填写完整的信息",$ajax,"");	
		}
		
		$GLOBALS['db']->query("update ".DB_PREFIX."user set ex_qq = '".$ex_qq."',ex_account_bank = '".$ex_account_bank."',ex_real_name = '".$ex_real_name."',ex_account_info = '".$ex_account_info."',ex_contact = '".$ex_contact."' where id = ".intval($GLOBALS['user_info']['id']));
