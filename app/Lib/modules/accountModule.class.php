...
class accountModule extends BaseModule
{
	...
	public function submitrefund()
	{
		...
		$user_r_name = strim($_REQUEST['user_r_name']);
		$bank = strim($_REQUEST['bank']);
		...
		$refund_data['user_r_name'] = $user_r_name;
		$refund_data['bank'] = $bank;
		$refund_data['account'] = $account;
		$refund_data['refund_type']=strim($_REQUEST['refund_type']);
		
		$GLOBALS['db']->autoExecute(DB_PREFIX."user_refund",$refund_data);
				
		showSuccess("申请成功，请等候审核！",$ajax,get_gopreview());
	}
